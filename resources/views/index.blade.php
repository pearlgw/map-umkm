...
<body>
    ...
    <input type="text" id="searchBox" placeholder="🔍 Cari lokasi..."
        class="absolute top-[15px] left-1/2 -translate-x-1/2 z-[1000] p-2 w-[250px] rounded-lg border border-gray-300 text-sm shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-300">
    ...
    <script>
        var map = L.map('map', {
            center: [-7.150975, 110.140259], // Titik tengah Jateng
            zoom: 9,
            maxBounds: [
                [-8.5, 107.9], // Barat daya (Brebes & Cilacap ke barat)
                [-5.5, 112.5] // Timur laut (Rembang ke timur)
            ],
            maxBoundsViscosity: 1.0, // Membatasi agar tidak bisa digeser keluar batas
            worldCopyJump: false,
            dragging: true
        });
            ...
            var marker = L.marker(coords, {
                ...
                .bindPopup(`
                    <div style="text-align: center;">
                        <center>
                            <img src="${city.image_url}" alt="${city.name}" style="width: 60px; margin-bottom: 11px;">
                        </center>
                        <b style="font-size: 16px;">${city.name}</b><br>
                        <span style="font-size: 15px;">Jumlah UMKM: ${city.umkms_count}</span><br>
                        <a href="/detail-umkm/${city.id}" 
                        style="color: black; text-decoration: underline; font-size: 15px;">
                            Lihat Data UMKM di ${city.name}
                        </a>
                    </div>
                `)
                .addTo(map);
